...
import (
	...
	"github.com/Localapak/localapak-backend/pkg/validator"
)
...
func main() {
	...
	// Recruitment use cases & handlers
	jobUseCase := usecase.NewJobUseCase(jobRepo, appRepo)
	jobHandler := handler.NewJobHandler(jobUseCase)

	// Initialize router
	r := router.NewRouter(userHandler, healthHandler, jwtManager, jobHandler, cfg.App.Debug)
